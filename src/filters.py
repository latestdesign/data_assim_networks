...
class DAN(nn.Module):
    ...
    def forward(self, ha, x, y):
        ...
        # translate analyzed mem into analyzed pdf
        pdf_a = self.c(ha)
        # translate post mem into post pdf
        logpdf_a = pdf_a.log_prob(x)
        logpdf_b = pdf_b.log_prob(x)

        # TODO 2.4 rewrite loss
        loss = -(logpdf_a.mean() + logpdf_b.mean())
        ...
        with torch.no_grad():
            if logpdf_a is not None:
                ...
                self.scores["LOGPDF_b"].append(logpdf_b.mean().item())
                self.scores["LOGPDF_a"].append(logpdf_a.mean().item())
...
class Lin2d(nn.Module):
    # rotation dymnamics
    def __init__(self, x_dim, N, dt, init, theta=torch.pi/100, window=None):
        assert(x_dim == 2)
        nn.Module.__init__(self)
        # TODO 1.3
        # implement M
...
class EDO(nn.Module):
    ...
    def edo(self, x):
        ...
        |   |   |  |
        """
        v = torch.stack([torch.roll(x, shifts=-w, dims=1) for w in self.window], dim=2)
        v = v.view(-1, self.diameter)
        dx = (torch.nn.functional.bilinear(v, v, self.A)
            + torch.nn.functional.linear(v, self.b)
            + self.c)
        return dx.view(x.size())
...
class FcZero(nn.Module):
    ...
    def __init__(self, dim, deep, activation_classname):
        ...
        self.alphas = nn.Parameter(torch.zeros(deep)) # Error was here: we need to make alphas learnable parameters, else the ReZero trick does not do anything. Fix is to use nn.Parameter.
...
class Gaussian(nn.Module):
    ...
    def __init__(self, x_dim, vec, inds):
        ...
        self.covariance_matrix = torch.bmm(self.Lambda, self.Lambda.transpose(1,2))
        self.cov_inv_ = torch.cholesky_inverse(self.Lambda)
        self.cov_log_det_ = torch.sum(logcst + diaga, dim=1, keepdim=True)
    ...
    def log_prob(self, x):
        ...
        #     term_1 = -0.5*mean_diff @ cov_inv @ mean_diff.unsqueeze(1)
        #     term_2 = -self.cov_log_det_[i,0]
        #     logprob[i,0] = term_1 + term_2

        # More efficient vectorized version
        mean_diff = x - self.mu
        term_1 = -0.5 * mean_diff.unsqueeze(1) @ self.cov_inv_ @ mean_diff.unsqueeze(2)
        # we convert the dim_x vectors into matrices of shape (1, dim_x) and (dim_x, 1) to perform the matrix multiplication with cov_inv_
        term_1 = term_1.squeeze(2) # And convert back to a batch of scalars
