...
def get_x0_test(b_size, x_dim, sigma):
    """Same as get_x0, but the result is stored in _x0_test."""
...
def pre_train_full(net,b_size,h_dim,x_dim,
    ...
    print('empirical mean of x0 is', x0.mean(axis=0))
    ...
    def closure0():
        ...
        logpdf_a0 = -net.c(ha0).log_prob(x0).mean()
        logpdf_a0.backward()
        nonlocal ite # a counter of number of evaluations
        ite = ite + 1
        return logpdf_a0
...
def train_full(net, b_size, h_dim, x_dim,
    ...
    def closure():
        ...
        # L_t(q^b_t) + L_t(q^a_t)
        for t in range(T):
            loss_t, ha = net(ha, xt[t], yt[t])
            loss_total += loss_t

        loss_0 = -net.c(ha0).log_prob(x0).mean() # L_0(q^a_0)
        loss_total = loss_total / T + loss_0
        loss_total.backward()
...
def train_online(net, b_size, h_dim, x_dim,
    ...
    for t in range(1, T+1):
        ...
        # Truncate the back propagation through time
        ha = ha.detach()
...
def save_dict(prefix, **kwargs):
    ...
    """
    # Note: I tweaked the function to avoid saving model weights in txt format (for the lorenz exp)
    for key, val in kwargs.items():
        if isinstance(val, dict):
            # Score-like dicts: pt for full history + txt for quick reading
            torch.save(val, prefix + key + ".pt")
            lines = []
            for key2, val2 in val.items():
                if isinstance(val2, (list, tuple)) and len(val2) > 0:
                    lines.append(key2 + '\t' + str(val2[-1]) + '\n')
            if len(lines) > 0:
                with open(prefix + key + '.txt', 'w') as f:
                    f.writelines(lines)
        else:
            # Weights/tensors: pt only
            torch.save(val, prefix + key + ".pt")
